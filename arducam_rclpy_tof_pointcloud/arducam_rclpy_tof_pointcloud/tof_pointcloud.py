...
class TOFPublisher(Node):
    def __init__(self, options: Option):
        ...
        self.publisher_ = self.create_publisher(PointCloud2, "point_cloud", 10) # 10 or qos
        ...
        self.header = Header()
        self.header.frame_id = "depth_camera_link_optical"
        self.points = None
        self.running_ = True
    ...
    def __generateSensorPointCloud(self):
        while self.running_:
            ...
            if frame is not None and isinstance(frame, DepthData):
                ...
                #self.depth_msg_.data = depth_buf.flatten() / 1000.0
                self.depth_msg_.data = (depth_buf.flatten() / 1000.0).astype('float32').tolist()
                ...
                #print(f"Got {len(self.points)} points.")  # debug
                

                self.tof_.releaseFrame(frame)

    def update(self):
        #print("Updating publisher...")  # debug
        # self.__generateSensorPointCloud()
        if self.points is None:
            print("No points yet!")  # debug
            return
...
def main(args=None):
    ...
    try:
        ...
        tof_publisher.stop()  # Ensure this line runs
